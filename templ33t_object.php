		...
		if ($this->render) {
			...
			if (!empty($this->block_objects)) {
				...
				foreach ($this->block_objects as $slug => $block) {
					...
					$descs .= '<div class="'. (!empty($block->description) || !empty($block->default) ? 'templ33t_description ' : '') . 'templ33t_desc_' . $block->id . ' templ33t_hidden"><p>' . $block->description . (!empty($block->default) ? ' (Leave empty for default value)' : '') . '</p></div>';
