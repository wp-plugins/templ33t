				...
				foreach($this->map[$theme] as $template) {
					...
					foreach($template['blocks'] as $slug => $config) {
						...
						if($config['customize_page']) {
							...
							$this->load_plugs[$config['type']] = $config['type'];
		...
		if ($this->render) {
			...
			if (!empty($this->block_objects)) {
				...
				foreach ($this->block_objects as $slug => $block) {
					...
					$descs .= '<div class="templ33t_description templ33t_desc_' . $block->id . ' templ33t_hidden"><p>' . (!empty($block->description) ? $block->description : 'Please enter your content below') . (!empty($block->default) ? ' (Leave empty for default value)' : '') . '</p></div>';
